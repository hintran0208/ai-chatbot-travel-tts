...
def text_to_speech(text: str, max_length: int = 200) -> Optional[str]:
    ...
    try:
        ...
        tmp_file_path = os.path.join(tempfile.gettempdir(), f"tts_audio_{unique_id}.wav")
        
        # Write the waveform to the temporary file
        sf.write(tmp_file_path, waveform, 16000)  # 16kHz sample rate
        ...
        # Clean up temporary file in the finally block to ensure it runs
        ...
        if tmp_file_path and os.path.exists(tmp_file_path):
            try:
                ...
                # Not critical if we can't delete the temp file, it will be cleaned up eventually
...
class ChatRequest(BaseModel):
    ...
    personalized: bool = Field(False, description="Whether to use personalized user data for better recommendations")
...
- When personalization is enabled, I can access your detailed profile including travel history, preferences, dietary needs, loyalty programs, and special requirements for highly tailored recommendations
...
async def process_chat_message(message: str, conversation_id: str, personalized: bool = False) -> tuple[str, List[Dict], Optional[str]]:
    ...
    if personalized:
        ...
        travel_history_str = ', '.join([f"{trip['destination']} ({trip['purpose']})" for trip in user_mock_data['travel_history'][-3:]])
        enhanced_context += f"- Recent Travel History: {travel_history_str}\n"
        enhanced_context += f"- Loyalty Programs: {', '.join(user_mock_data['loyalty_programs'])}\n"
        enhanced_context += f"- Last Search: {user_mock_data['last_search']['destination']} for {user_mock_data['last_search']['date']}\n"
    ...
    try:
        while True:
            ...
            if user_message.strip():
                # Process the message
                response, function_calls, audio_base64 = await process_chat_message(user_message, conversation_id, personalized)
...
async def chat_api(request: ChatRequest):
    ...
    try:
        response, function_calls, audio_base64 = await process_chat_message(request.message, conversation_id, request.personalized)
